...
- **Export Dedup Removed — Feb 2026:** Removed aggressive export-time deduplication that was dropping valid loan records (887→542 in Aug export). Export now returns all records matching the month group, exactly matching what MIS board shows. Admin cleanup endpoint `POST /api/loans/cleanup-duplicates` remains available for manual DB dedup if needed.
