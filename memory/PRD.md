...
- **Group Month Backfill Migration — Feb 2026:** Startup migration automatically sets `group_month` for orphan records (missing/empty) by deriving it from the date field. Ensures all records have explicit `group_month`, eliminating date-based fallback guessing. Frontend limit increased to 10000 to show all data. Backend cap also 10000.
