...
- **Export Dedup Removed & Strict Group Filter — Feb 2026:** Removed aggressive export-time deduplication that was dropping valid loan records. Export now uses strict `group_month` match only (no legacy fallback regex), so it returns exactly the same records the MIS board shows. Admin cleanup endpoint `POST /api/loans/cleanup-duplicates` remains available for manual DB dedup if needed.
