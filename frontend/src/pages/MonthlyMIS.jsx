...
const MonthlyMIS = () => {
  ...
  const getTableWidth = () => {
    ...
    let total = (showBulkSelect ? CHECKBOX_COL_W : 0) + ACTION_COL_W;
    for (const col of visibleCols) {
      total += COL_WIDTHS[col.key] || DEFAULT_COL_W;
    }
    return total;
  };
  ...
  const getFreezeStyle = (key) => {
    if (!FREEZE_KEYS.includes(key)) {
      const w = COL_WIDTHS[key] || DEFAULT_COL_W;
      return { width: `${w}px`, minWidth: `${w}px` };
    }
    const visibleCols = ALL_COLUMNS.filter(c => visibleColumns.includes(c.key));
    let left = showBulkSelect ? 32 : 0;
    for (const col of visibleCols) {
      if (col.key === key) break;
      if (FREEZE_KEYS.includes(col.key)) {
  ...
  const ALL_COLUMNS = [
    ...
    { key: 'sanction', label: 'Sanction Amount' },
    { key: 'disbursed', label: 'Disbursed Amount' },
  ...
  // Inline master add dialog
  const [showInlineAdd, setShowInlineAdd] = useState(false);
  const [inlineAddType, setInlineAddType] = useState(''); // 'customer', 'executive', 'manager'
  const [inlineAddName, setInlineAddName] = useState('');
  const [inlineAddContact, setInlineAddContact] = useState('');
  const [inlineAddTarget, setInlineAddTarget] = useState(''); // 'add' or 'edit' form

  const openInlineAdd = (type, target) => {
    setInlineAddType(type);
    setInlineAddTarget(target);
    setInlineAddName('');
    setInlineAddContact('');
  ...
  const handleInlineAddSave = async () => {
    ...
    try {
      const endpointMap = { customer: 'customers', executive: 'executives', manager: 'managers' };
      const payload = { name: inlineAddName.trim() };
      if (inlineAddType === 'customer') payload.contact_no = inlineAddContact.trim();
      const res = await axios.post(`${API}/master/${endpointMap[inlineAddType]}`, payload);
      // Update master list
      if (inlineAddType === 'customer') setMasterCustomers(prev => [...prev, res.data].sort((a, b) => a.name.localeCompare(b.name)));
      if (inlineAddType === 'executive') setMasterExecutives(prev => [...prev, res.data].sort((a, b) => a.name.localeCompare(b.name)));
      if (inlineAddType === 'manager') setMasterManagers(prev => [...prev, res.data].sort((a, b) => a.name.localeCompare(b.name)));
      // Auto-select in the target form
      const fieldMap = { customer: 'customer_name', executive: 'executive_name', manager: 'team_manager' };
      if (inlineAddTarget === 'add') {
        const updates = { [fieldMap[inlineAddType]]: res.data.name };
        if (inlineAddType === 'customer' && res.data.contact_no) updates.contact_no = res.data.contact_no;
        setNewLoanData(prev => ({ ...prev, ...updates }));
      } else {
        const updates = { [fieldMap[inlineAddType]]: res.data.name };
        if (inlineAddType === 'customer' && res.data.contact_no) updates.contact_no = res.data.contact_no;
        setEditFormData(prev => ({ ...prev, ...updates }));
      }
      toast.success(`${inlineAddName.trim()} added`);
  ...
  const calculateMonthTotals = (loans) => {
    ...
    loans.forEach(loan => {
      ...
      totals.amount += parseNum(loan.amount);
  ...
  const filteredLoans = loans.filter(loan => {
    ...
    if (filterBranches.length > 0 && !filterBranches.includes(loan.branch)) return false;
    if (filterLocations.length > 0 && !filterLocations.includes(loan.location)) return false;
  ...
  const renderCell = (loan, field, label) => {
    ...
    return (
      <div className="px-1.5 py-0.5 min-h-[24px] flex items-center text-[11px]">
    ...
    <div className="space-y-3 fade-in pb-16" data-testid="mis-board-page">
      ...
      <div className="flex flex-wrap items-center gap-2" data-testid="quick-filters">
        ...
        {(filterCategories.length > 0 || filterProducts.length > 0 || filterBanks.length > 0 || filterStatuses.length > 0 || filterCompanies.length > 0 || filterCaseFrom.length > 0 || filterManagers.length > 0 || filterExecutives.length > 0 || filterBranches.length > 0 || filterLocations.length > 0) && (
          <Button variant="ghost" size="sm" onClick={() => { setFilterCategories([]); setFilterProducts([]); setFilterBanks([]); setFilterStatuses([]); setFilterCompanies([]); setFilterCaseFrom([]); setFilterManagers([]); setFilterExecutives([]); setFilterBranches([]); setFilterLocations([]); }}
        ...
        {(filterCategories.length > 0 || filterProducts.length > 0 || filterBanks.length > 0 || filterStatuses.length > 0 || filterCompanies.length > 0 || filterCaseFrom.length > 0 || filterManagers.length > 0 || filterExecutives.length > 0 || filterBranches.length > 0 || filterLocations.length > 0) && (
          <div className="flex flex-wrap gap-1 ml-1">
            ...
            {filterBranches.map(b => <span key={b} className="px-1.5 py-0.5 rounded-full bg-indigo-100 text-[9px] text-indigo-700 font-medium cursor-pointer hover:bg-red-100 hover:text-red-600" onClick={() => setFilterBranches(filterBranches.filter(x => x !== b))}>{b} &times;</span>)}
            {filterLocations.map(l => <span key={l} className="px-1.5 py-0.5 rounded-full bg-lime-100 text-[9px] text-lime-700 font-medium cursor-pointer hover:bg-red-100 hover:text-red-600" onClick={() => setFilterLocations(filterLocations.filter(x => x !== l))}>{l} &times;</span>)}
          </div>
        )}
      </div>
      )}
            ...
            <div key={month} className="border border-slate-200 rounded-lg bg-white shadow-sm">
              ...
              {isExpanded && (
                <div className="overflow-x-auto" data-testid={`mis-table-scroll-${month}`}>
                  <table className="text-[11px]" style={{ tableLayout: 'fixed', width: `${getTableWidth()}px` }}>
                    <thead>
                      <tr className="border-b border-slate-200 bg-[#2c587a]/5">
                        {showBulkSelect && (
                        <th className="px-2 py-1.5 bg-slate-50" style={{ position: 'sticky', left: 0, zIndex: 10, width: `${CHECKBOX_COL_W}px`, minWidth: `${CHECKBOX_COL_W}px` }}>
                          <input
                            type="checkbox"
                        ...
                        <th className="px-2 py-1.5 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap" style={{ width: `${ACTION_COL_W}px`, minWidth: `${ACTION_COL_W}px` }}></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {monthLoans.map(loan => (
                        <tr key={loan.id} data-testid={`mis-row-${loan.id}`} className={`transition-colors ${selectedIds.has(loan.id) ? 'bg-blue-50/60' : 'hover:bg-slate-50/50'}`}>
                          ...
                          <td className="px-2 py-1 whitespace-nowrap sticky right-0 bg-white" style={{ width: `${ACTION_COL_W}px`, minWidth: `${ACTION_COL_W}px` }}>
          ...
          <form onSubmit={handleAddLoan} className="space-y-3">
            <div className="grid grid-cols-2 gap-x-3 gap-y-2">
              <div>
                ...
                <div className="flex gap-1 mt-0.5">
                  <Select key={`cust-add-${masterCustomers.length}`} value={newLoanData.customer_name || ''} onValueChange={(value) => {
                    ...
                    setNewLoanData(prev => ({...prev, customer_name: value, ...(cust?.contact_no ? { contact_no: cust.contact_no } : {})}));
                  }}>
                    <SelectTrigger className="h-8 text-[11px] flex-1" data-testid="add-customer-select"><SelectValue placeholder="Select customer" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="__add_new__" className="text-[11px] text-[#2c587a] font-medium">+ Add New Customer</SelectItem>
                      {newLoanData.customer_name && !masterCustomers.some(c => c.name === newLoanData.customer_name) && (
                        <SelectItem value={newLoanData.customer_name} className="text-[11px]">{newLoanData.customer_name}</SelectItem>
                      )}
              ...
              <div>
                ...
                <div className="flex gap-1 mt-0.5">
                  <Select key={`exec-add-${masterExecutives.length}`} value={newLoanData.executive_name || ''} onValueChange={(value) => {
                    if (!value || value === '') return;
                    if (value === '__add_new__') { openInlineAdd('executive', 'add'); return; }
                    setNewLoanData(prev => ({...prev, executive_name: value}));
                  }}>
                    <SelectTrigger className="h-8 text-[11px] flex-1" data-testid="add-executive-select"><SelectValue placeholder="Select executive" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="__add_new__" className="text-[11px] text-[#2c587a] font-medium">+ Add New Executive</SelectItem>
                      {newLoanData.executive_name && !masterExecutives.some(e => e.name === newLoanData.executive_name) && (
                        <SelectItem value={newLoanData.executive_name} className="text-[11px]">{newLoanData.executive_name}</SelectItem>
                      )}
              ...
              <div>
                ...
                <div className="flex gap-1 mt-0.5">
                  <Select key={`mgr-add-${masterManagers.length}`} value={newLoanData.team_manager || ''} onValueChange={(value) => {
                    if (!value || value === '') return;
                    if (value === '__add_new__') { openInlineAdd('manager', 'add'); return; }
                    setNewLoanData(prev => ({...prev, team_manager: value}));
                  }}>
                    <SelectTrigger className="h-8 text-[11px] flex-1" data-testid="add-manager-select"><SelectValue placeholder="Select manager" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="__add_new__" className="text-[11px] text-[#2c587a] font-medium">+ Add New Manager</SelectItem>
                      {newLoanData.team_manager && !masterManagers.some(m => m.name === newLoanData.team_manager) && (
                        <SelectItem value={newLoanData.team_manager} className="text-[11px]">{newLoanData.team_manager}</SelectItem>
                      )}
          ...
          <form onSubmit={handleEditSave} className="space-y-3">
            <div className="grid grid-cols-2 gap-x-3 gap-y-2">
              <div>
                ...
                <div className="flex gap-1 mt-0.5">
                  <Select key={`cust-edit-${masterCustomers.length}`} value={editFormData.customer_name || ''} onValueChange={(value) => {
                      ...
                      const cust = masterCustomers.find(c => c.name === value);
                      setEditFormData(prev => ({...prev, customer_name: value, ...(cust?.contact_no ? { contact_no: cust.contact_no } : {})}));
                    }}>
                      <SelectTrigger className="h-8 text-[11px] flex-1" data-testid="edit-customer-select"><SelectValue placeholder="Select customer" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__add_new__" className="text-[11px] text-[#2c587a] font-medium">+ Add New Customer</SelectItem>
                  ...
                  <button type="button" onClick={() => openInlineAdd('customer', 'edit')} className="h-8 w-8 flex items-center justify-center rounded-md border border-slate-200 hover:bg-[#2c587a]/10 transition-colors shrink-0" title="Add new customer" data-testid="edit-inline-add-customer-btn"><Plus className="w-3.5 h-3.5 text-[#2c587a]" /></button>
                </div>
              </div>
              <div>
                <Label className="text-[11px] text-slate-600">Contact Number *</Label>
              ...
              <div>
                <Label className="text-[11px] text-slate-600">Executive Name</Label>
                <div className="flex gap-1 mt-0.5">
                  <Select key={`exec-edit-${masterExecutives.length}`} value={editFormData.executive_name || ''} onValueChange={(value) => {
                      if (!value || value === '') return;
                      if (value === '__add_new__') { openInlineAdd('executive', 'edit'); return; }
                      setEditFormData(prev => ({...prev, executive_name: value}));
                    }}>
                      <SelectTrigger className="h-8 text-[11px] flex-1" data-testid="edit-executive-select"><SelectValue placeholder="Select executive" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__add_new__" className="text-[11px] text-[#2c587a] font-medium">+ Add New Executive</SelectItem>
                  ...
                  <button type="button" onClick={() => openInlineAdd('executive', 'edit')} className="h-8 w-8 flex items-center justify-center rounded-md border border-slate-200 hover:bg-[#2c587a]/10 transition-colors shrink-0" title="Add new executive" data-testid="edit-inline-add-executive-btn"><Plus className="w-3.5 h-3.5 text-[#2c587a]" /></button>
                </div>
              </div>
              <div>
                <Label className="text-[11px] text-slate-600">Team Manager</Label>
                <div className="flex gap-1 mt-0.5">
                  <Select key={`mgr-edit-${masterManagers.length}`} value={editFormData.team_manager || ''} onValueChange={(value) => {
                      if (!value || value === '') return;
                      if (value === '__add_new__') { openInlineAdd('manager', 'edit'); return; }
                      setEditFormData(prev => ({...prev, team_manager: value}));
                    }}>
                      <SelectTrigger className="h-8 text-[11px] flex-1" data-testid="edit-manager-select"><SelectValue placeholder="Select manager" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__add_new__" className="text-[11px] text-[#2c587a] font-medium">+ Add New Manager</SelectItem>
                  ...
                  <button type="button" onClick={() => openInlineAdd('manager', 'edit')} className="h-8 w-8 flex items-center justify-center rounded-md border border-slate-200 hover:bg-[#2c587a]/10 transition-colors shrink-0" title="Add new manager" data-testid="edit-inline-add-manager-btn"><Plus className="w-3.5 h-3.5 text-[#2c587a]" /></button>
                </div>
              </div>
              <div>
                <Label className="text-[11px] text-slate-600">Company Name *</Label>
      ...
      {/* Inline Add Master Data Dialog */}
      <Dialog open={showInlineAdd} onOpenChange={setShowInlineAdd}>
        <DialogContent className="max-w-xs">
          <DialogHeader>
            <DialogTitle className="text-sm">
              Add New {inlineAddType === 'customer' ? 'Customer' : inlineAddType === 'executive' ? 'Executive' : 'Manager'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Input
                ...
                placeholder={inlineAddType === 'customer' ? 'e.g., Rahul Sharma' : inlineAddType === 'executive' ? 'e.g., Amit Patel' : 'e.g., Priya Singh'}
                className="h-8 text-[11px] mt-0.5"
                data-testid="inline-add-name-input"
              />
            </div>
            {inlineAddType === 'customer' && (
              <div>
                <Label className="text-[11px] text-slate-600">Contact Number</Label>
                <Input
                  value={inlineAddContact}
                  onChange={(e) => setInlineAddContact(e.target.value)}
                  placeholder="e.g., 9876543210"
                  className="h-8 text-[11px] mt-0.5"
                  data-testid="inline-add-contact-input"
                />
              </div>
            )}
            <div className="flex gap-2 justify-end">
              <Button type="button" variant="outline" size="sm" onClick={() => setShowInlineAdd(false)} className="h-7 text-[11px]">Cancel</Button>
              <Button type="button" size="sm" onClick={handleInlineAddSave} className="h-7 text-[11px] bg-[#2c587a] hover:bg-[#234a68]" data-testid="inline-add-save-btn">Add</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
